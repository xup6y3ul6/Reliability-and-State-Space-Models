...
transformed data {
  ...
  for (n in 1:N) {
    m_0[n] = rep_vector(0.0, P);
    C_0[n] = diag_matrix(rep_vector(100.0, P));
  }
...
parameters {
  ...
  vector<lower=0>[P * (P + 1) %/% 2] diag_Psi_R; // prior covariance of the covariance of the measurement error
  vector[P * (P + 1) %/% 2] gamma_Q; // prior mean of the covariance of the innovation noise
  vector<lower=0>[P * (P + 1) %/% 2] diag_Psi_Q; // prior covariance of the covariance of innovation noise
}
...
transformed parameters {
  ...
  for (p in 1:P) {
    mu_R[p] = exp(gamma_R[index_of_diag_lower_tri(p, P)] +  diag_Psi_R[index_of_diag_lower_tri(p, P)]^2 / 2.0);
  }
}
...
model {
  ...
  for (n in 1:N) {
    ...
    to_vector_lower_tri(R[n]) ~ lognormal(gamma_R, sqrt(diag_Psi_R));
    to_vector_lower_tri(Q[n]) ~ lognormal(gamma_Q, sqrt(diag_Psi_Q));
  }
